...
SVC_CHANNELS = {"JTBC2 (SVC)", "SPOTV G&H (SVC)", "히스토리 (SVC)", "기타 TV채널", "발신"}
SVC_ORDER    = ["발신", "히스토리 (SVC)", "SPOTV G&H (SVC)", "JTBC2 (SVC)", "기타 TV채널"]
...
def update_html(daily, channel, other, material, kpi, template, output):
    with open(template, encoding="utf-8") as f:
        html = f.read()

    # JS 배열 문자열 생성
    daily_js    = "const DAILY=[\n"    + ",\n".join(" " + to_js_obj(d) for d in daily)    + "\n];"
    channel_js  = "const CHANNEL=[\n"  + ",\n".join(" " + to_js_obj(c) for c in channel)  + "\n];"
    other_js    = "const OTHER=[\n"    + ",\n".join(" " + to_js_obj(o) for o in other)    + "\n];"
    material_js = "const MATERIAL=[\n" + ",\n".join(" " + to_js_obj(m) for m in material) + "\n];"
    kpi_js      = "const KPI=" + to_js_obj(kpi) + ";"

    # HTML 안의 데이터 블록 교체
    html = re.sub(r"const DAILY=\[[\s\S]*?\];",    daily_js,    html)
    html = re.sub(r"const CHANNEL=\[[\s\S]*?\];",  channel_js,  html)
    html = re.sub(r"const OTHER=\[[\s\S]*?\];",    other_js,    html)
    html = re.sub(r"const MATERIAL=\[[\s\S]*?\];", material_js, html)
    html = re.sub(r"const KPI=\{[\s\S]*?\};",      kpi_js,      html)
...
def main():
    ...
    channel  = build_channel(rows)
    other    = build_other(rows)
    material = build_material(rows)
    kpi      = build_kpi(channel, other, daily)

    print("[3] HTML 업데이트 중...")
    update_html(daily, channel, other, material, kpi, template, output)

    print(f"\n✅ 완료! → {output}")
    if daily:
        print(f"   기간    : {daily[0]['date']} ~ {daily[-1]['date']} ({len(daily)}일)")
    print(f"   총광고비 : ₩{kpi['총광고비']:,}")
    print(f"   총인입콜 : {kpi['총인입콜']:,}건")
    print(f"   총정기건수: {kpi['총정기건수']:,}건")
    print(f"   총정기금액: ₩{kpi['총정기금액']:,}")
    print(f"   ROI      : {kpi['ROI']}  |  CPR: ₩{kpi['CPR']:,}")
