...
def is_svc(ch):
    """채널명에 (SVC) 포함 또는 고정 SVC 채널이면 True — 새 채널 추가 시 자동 인식"""
    return bool(re.search(r"\(SVC\)", ch, re.IGNORECASE)) or ch in {"기타 TV채널", "발신", "웹"}

# 하위 호환: 기존 코드에서 `ch in SVC_CHANNELS` 쓰는 곳은 is_svc(ch)로 대체
SVC_CHANNELS = None   # 더 이상 직접 사용하지 않음
SVC_ORDER_PREFIX = ["발신", "기타 TV채널", "웹"]  # 고정 순서 앞부분
...
def build_channel(rows):
    """채널+소재 단위로 집계. JTBC4는 전체 합산 후 소재='MIX'로 표시."""
    CHANNEL_MIX = {"JTBC4"}   # 소재 합산할 채널 목록
    agg = defaultdict(lambda: {
        "광고비": 0, "광고횟수": 0, "인입콜": 0,
        "응대콜": 0, "정기건수": 0, "정기금액": 0,
    })
    for r in rows:
        ...
        mat = r.get("소재", "").strip() or "-"
        mat = mat.replace("은영이 ver2", "은영이ver2")
        if not ch or is_svc(ch):
            continue
        # MIX 채널은 소재 무시하고 채널 단위로 합산
        if ch in CHANNEL_MIX:
            mat = "MIX"
        key = (ch, mat)
        a = agg[key]
        a["광고비"]   += to_num(r.get("광고비"))
    ...
    # 채널별 집계 ROI (비우선 채널 정렬용)
    ch_agg = defaultdict(lambda: {"광고비": 0, "정기금액": 0})
    for (ch, mat), v in agg.items():
        ch_agg[ch]["광고비"]   += v["광고비"]
        ch_agg[ch]["정기금액"] += v["정기금액"]

    def ch_roi(ch):
        a = ch_agg[ch]
        return (a["정기금액"] * 12 / a["광고비"]) if a["광고비"] else 0

    result = []
    for (ch, mat), v in sorted(
        agg.items(),
        key=lambda x: (-ch_roi(x[0][0]), x[0][0], x[0][1])
    ):
        ad   = v["광고비"]
        cnt  = v["광고횟수"]
        inc  = v["인입콜"]
...
def build_other(rows):
    agg = defaultdict(lambda: {
        "광고횟수": 0, "인입콜": 0, "응대콜": 0, "정기건수": 0, "정기금액": 0, "days": set(), "소재": set(),
    })
    for r in rows:
        ch = r.get("Channel", "").strip()
        if not is_svc(ch):
    ...
    # SVC 채널 정렬: 고정 앞부분 우선, 나머지는 이름순 자동 추가
    fixed   = [c for c in SVC_ORDER_PREFIX if c in agg]
    dynamic = sorted(c for c in agg if c not in SVC_ORDER_PREFIX)
    svc_order = fixed + dynamic
    ...
    for ch in svc_order:
        ...
        콜전환율  = round(inc  / cnt  * 100, 1) if cnt  else 0
        정기전환율 = round(reg  / resp * 100, 1) if resp else 0
        소재명 = ", ".join(sorted(v["소재"])) if v["소재"] else "-"
        result.append({
            "ch":       ch,
            "소재":     소재명,
            "인입콜":   int(inc),
